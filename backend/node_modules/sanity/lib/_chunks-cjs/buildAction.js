...
async function buildVendorDependencies({
  ...
  for (const [packageName, ranges] of Object.entries(VENDOR_IMPORTS)) {
    ...
    try {
      ...
      throw typeof e?.message == "string" ? (e.message = `${message}: ${e.message}`, e) : new Error(message, { cause: e });
    }
...
async function buildSanityStudio(args, context, overrides) {
  ...
  if (didInstall)
    return { didCompile: !1 };
  const autoUpdatesEnabled = flags["auto-updates"] || cliConfig && "autoUpdates" in cliConfig && cliConfig.autoUpdates === !0, coercedSanityVersion = semver__default.default.coerce(installedSanityVersion)?.version;
...
Including the following environment variables as part of the JavaScript bundle:`
  ...
  overrides?.basePath ? basePath = overrides.basePath : envBasePath ? basePath = envBasePath : configBasePath && (basePath = configBasePath), envBasePath && configBasePath && output.warn(
