"use strict";
var path = require("node:path"), zlib = require("node:zlib"), tar = require("tar-fs"), buildAction = require("./buildAction.js"), helpers = require("./helpers.js");
...
async function deployStudioAction(args, context) {
  const { apiClient, workDir, chalk, output, prompt, cliConfig } = context, flags = { build: !0, ...args.extOptions }, customSourceDir = args.argsWithoutOptions[0], sourceDir = path__default.default.resolve(process.cwd(), customSourceDir || path__default.default.join(workDir, "dist")), isAutoUpdating = flags["auto-updates"] || cliConfig && "autoUpdates" in cliConfig && cliConfig.autoUpdates === !0 || !1, installedSanityVersion = await helpers.getInstalledSanityVersion(), configStudioHost = cliConfig && "studioHost" in cliConfig && cliConfig.studioHost, client = apiClient({
    requireUser: !0,
    requireProject: !0
...
`);
  ...
  if (flags.build) {
    const buildArgs = [customSourceDir].filter(Boolean), { didCompile } = await buildAction.default(
  ...
  try {
    const { location } = await helpers.createDeployment({
      client,
