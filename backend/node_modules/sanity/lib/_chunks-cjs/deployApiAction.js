...
const SCHEMA_LIFT_ANONYMOUS_OBJECT_TYPE = "schema-lift-anonymous-object-type", consoleOutputter = { error: (...args) => console.error(...args) };
...
function extractFromSanitySchema(sanitySchema, extractOptions = {}) {
  ...
  function getArrayDefinition(def, parent, options = {}) {
    const base = { description: getDescription(def), kind: "List" }, name = !options.isField && def.name ? { name: getTypeName(def.name) } : {}, candidates = def.type?.type && "of" in def.type ? arrayify(def.type.of) : def.of;
