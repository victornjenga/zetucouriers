...
function PaneRouterProvider(props) {
  const { children, flatIndex, index, params, payload, siblingIndex } = props, { navigate, navigateIntent, resolvePathFromState } = router.useRouter(), routerState = router.useRouterState(), { panes, expand } = structure.usePaneLayout(), routerPaneGroups = React.useMemo(
    ...
    [routerState?.panes]
  ), lastPane = React.useMemo(() => panes?.[panes.length - 2], [panes]), groupIndex = index - 1, createNextRouterState = React.useCallback(
    (modifier) => {
...
function useRouterPanesStream() {
  const routerStateSubject = React.useMemo(() => new rxjs.ReplaySubject(1), []), routerPanes$ = React.useMemo(
    () => routerStateSubject.asObservable().pipe(operators.map((_routerState) => _routerState?.panes || [])),
    [routerStateSubject]
...
function StructureError({ error }) {
  if (!(error instanceof PaneResolutionError))
    throw error;
  const { cause } = error, { t } = sanity.useTranslation(structure.structureLocaleNamespace), stack = cause?.stack || error.stack, showStack = stack && !(cause instanceof structure.SerializeError) && !error.message.includes("Module build failed:"), path = cause instanceof structure.SerializeError ? cause.path : [], helpId = cause instanceof structure.SerializeError && cause.helpId || error.helpId, handleReload = React.useCallback(() => {
...
const DocumentTitle = (props) => {
  const { documentId, documentType } = props, editState = sanity.useEditState(documentId, documentType), schema = sanity.useSchema(), { t } = sanity.useTranslation(structure.structureLocaleNamespace), isNewDocument = !editState?.published && !editState?.draft, documentValue = editState?.draft || editState?.published, schemaType = schema.get(documentType), { value, isLoading: previewValueIsLoading } = sanity.unstable_useValuePreview({
    enabled: !0,
    schemaType,
    value: documentValue
  ...
  return isLoadingPane(lastPane) ? /* @__PURE__ */ jsxRuntime.jsx(PassthroughTitle, {}) : isDocumentPane(lastPane) ? lastPane?.title ? /* @__PURE__ */ jsxRuntime.jsx(PassthroughTitle, { title: lastPane.title }) : /* @__PURE__ */ jsxRuntime.jsx(DocumentTitle, { documentId: lastPane.options.id, documentType: lastPane.options.type }) : /* @__PURE__ */ jsxRuntime.jsx(PassthroughTitle, { title: lastPane?.title });
};
...
`, isSaveHotkey = isHotkeyEsm.isHotkey("mod+s"), StructureTool = React.memo(function({ onPaneChange }) {
  ...
  }, [pushToast]), schema._original?.types.some(sanity._isCustomDocumentTypeDefinition) ? /* @__PURE__ */ jsxRuntime.jsxs(ui.PortalProvider, { element: portalElement || null, children: [
