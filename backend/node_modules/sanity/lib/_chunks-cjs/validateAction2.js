...
function validateDocuments(options) {
  const {
    ...
    ndjsonFilePath
  } = options, rootPkgPath = readPkgUp__default.default.sync({ cwd: __dirname })?.path;
  if (!rootPkgPath)
  ...
  const workerPath = path__default.default.join(
    ...
    workerData: {
      ...
      level,
      ndjsonFilePath,
      maxCustomValidationConcurrency: maxCustomValidationConcurrency ?? DEFAULT_MAX_CUSTOM_VALIDATION_CONCURRENCY
    },
