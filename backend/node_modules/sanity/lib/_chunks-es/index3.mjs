...
function PaneRouterProvider(props) {
  const { children, flatIndex, index, params, payload, siblingIndex } = props, { navigate, navigateIntent, resolvePathFromState } = useRouter(), routerState = useRouterState(), { panes, expand } = usePaneLayout(), routerPaneGroups = useMemo(
    ...
    [routerState?.panes]
  ), lastPane = useMemo(() => panes?.[panes.length - 2], [panes]), groupIndex = index - 1, createNextRouterState = useCallback(
    (modifier) => {
...
function useRouterPanesStream() {
  const routerStateSubject = useMemo(() => new ReplaySubject(1), []), routerPanes$ = useMemo(
    () => routerStateSubject.asObservable().pipe(map((_routerState) => _routerState?.panes || [])),
    [routerStateSubject]
...
function StructureError({ error }) {
  if (!(error instanceof PaneResolutionError))
    throw error;
  const { cause } = error, { t } = useTranslation(structureLocaleNamespace), stack = cause?.stack || error.stack, showStack = stack && !(cause instanceof SerializeError) && !error.message.includes("Module build failed:"), path = cause instanceof SerializeError ? cause.path : [], helpId = cause instanceof SerializeError && cause.helpId || error.helpId, handleReload = useCallback(() => {
...
const DocumentTitle = (props) => {
  const { documentId, documentType } = props, editState = useEditState(documentId, documentType), schema = useSchema(), { t } = useTranslation(structureLocaleNamespace), isNewDocument = !editState?.published && !editState?.draft, documentValue = editState?.draft || editState?.published, schemaType = schema.get(documentType), { value, isLoading: previewValueIsLoading } = unstable_useValuePreview({
    enabled: !0,
    schemaType,
    value: documentValue
  ...
  return isLoadingPane(lastPane) ? /* @__PURE__ */ jsx(PassthroughTitle, {}) : isDocumentPane(lastPane) ? lastPane?.title ? /* @__PURE__ */ jsx(PassthroughTitle, { title: lastPane.title }) : /* @__PURE__ */ jsx(DocumentTitle, { documentId: lastPane.options.id, documentType: lastPane.options.type }) : /* @__PURE__ */ jsx(PassthroughTitle, { title: lastPane?.title });
};
...
`, isSaveHotkey = isHotkey("mod+s"), StructureTool = memo(function({ onPaneChange }) {
  ...
  }, [pushToast]), schema._original?.types.some(_isCustomDocumentTypeDefinition) ? /* @__PURE__ */ jsxs(PortalProvider, { element: portalElement || null, children: [
    /* @__PURE__ */ jsxs(
      StyledPaneLayout,
      {
        flex: 1,
